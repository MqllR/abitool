...
func newAppModel(initial screen) appModel {
	return appModel{
...
func (m homeModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	if msg, ok := msg.(tea.KeyMsg); ok {
		...
		case "enter", " ":
			switch m.cursor {
			case 0:
				next := newContractListModel(m.basePath)
...
func newContractListModel(basePath string) contractListModel {
	fi := textinput.New()
	...
	case tea.KeyMsg:
		...
		case "enter", " ":
			if len(m.filtered) > 0 && m.cursor < len(m.filtered) {
				...
				next := newBrowseModel(entry.address, entry.name, m.basePath)
				return m, func() tea.Msg { return pushMsg{next} }
			}
		}
	}
